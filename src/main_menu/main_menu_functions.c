...
main_menu_start_menu(void)
{
    ...
    return menu_creation_menu_option_chosen(options_names, num_options, NULL); 
}
    ...
    while ( ( option != QUIT_GAME ) && ( option != START_NEW_GAME ) && ( option != START_LOADED_GAME )  )
    {
        ...
        if ( option == NEW_GAME )
        {
            ...
            while ( ( option != RETURN_CREATE_CHARACTER ) && ( option != START_NEW_GAME ) )
            {
                ...
                else if ( option == CLASS_CREATE_CHARACTER ) class = main_menu_class_selection_menu();

                else if ( ( option == START_NEW_GAME && race == NONE_RACE ) || ( option == START_NEW_GAME && class == NONE_CLASS ) )
                {
