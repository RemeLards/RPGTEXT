...
float dmg_values_mobs(MOBS* monster)
{
    ...
    //Didn't Add Mana/Stamina Attacks in MOBS, And IDK Yet How Can I Make A Mob "Choose" a Random Ability
...
{
    int roll = rng_generator() % 101;//Random Number
    int attack_crit_chance = (character -> base_attackcritchance); //Chance of getting a basic attack crit
    int ability_crit_chance = (character -> base_abilitycritchance); //Chance of getting a ability crit
    float pure_damage = dmg_values_player(character,attack_status); //Getting a range of damage values

    //Player did not Crit with basic attack
    if((roll > attack_crit_chance)&& (attack_status == BASIC_ATTACK))
    {
        printf("\nDID A BASIC ATTACK!!!\n");
    ...
    if((roll <= attack_crit_chance) && (attack_status == BASIC_ATTACK))
    {
        ...
        float pure_critdmg = 1+(character-> base_attackcritdmg/100); //Basic Attack Crit Damage
        printf("\nDID A BASIC ATTACK!!!\n");
        printf("**CRIT DAMAGE**\n");
    ...
    if((roll > ability_crit_chance) && (attack_status == MANA_ATTACK || attack_status == STAMINA_ATTACK))
    {
